...
export const daysApi = {
  ...
  getAllDays: () => api.get<DayLog[]>("/days").then((r) => r.data),

  updateDay: (
    dateKey: string,
    payload: Partial<Pick<DayLog, "reflection" | "rating" | "displayDate">>
  ...
  updateHabit: (
    ...
    api
      .patch<DayLog>(`/days/${dateKey}/habits/${habitId}`, payload)
      .then((r) => r.data),

  deleteHabit: (dateKey: string, habitId: string) =>
    api
      .delete<DayLog>(`/days/${dateKey}/habits/${habitId}`)
      .then((r) => r.data),
};
