...
router.patch("/:dateKey", async (req, res) => {
    try {
        const { dateKey } = req.params;
        ...
        if (displayDate !== undefined)
            update.displayDate = displayDate;
        const day = await DayLog.findOneAndUpdate({ dateKey }, { $set: update }, { new: true });
        if (!day)
...
router.post("/:dateKey/habits", async (req, res) => {
    try {
        const { dateKey } = req.params;
        let { name } = req.body;
        ...
        name = (name || "").trim();
        if (!name)
            name = "New Habit"; // <-- Default fallback
...
router.patch("/:dateKey/habits/:habitId", async (req, res) => {
    try {
...
router.delete("/:dateKey/habits/:habitId", async (req, res) => {
    try {
        const { dateKey, habitId } = req.params;
        const day = await DayLog.findOne({ dateKey });
        if (!day)
...
router.put("/:dateKey/habits/reorder", async (req, res) => {
    try {
        const { dateKey } = req.params;
